		...
		for (int i = 0; i < OperationRetries; i++)
		{
			...
			try
			{
				...
				} else
				{
					...
					for (String s : fields)
					{
						ByteBuffer bb = ByteBufferUtil.EMPTY_BYTE_BUFFER.duplicate();
						bb.get(s.getBytes("UTF-8"));
						fieldlist.add(bb);
					}
				...
				ByteBuffer bb = ByteBufferUtil.EMPTY_BYTE_BUFFER.duplicate();
				bb.get(key.getBytes("UTF-8"));
				List<ColumnOrSuperColumn> results = client.get_slice(bb, parent, predicate,
	...
	public int insert(String table, String key, HashMap<String, String> values)
	{
		...
		for (int i = 0; i < OperationRetries; i++)
		{
			...
			try
			{
				...
				ByteBuffer b2 = ByteBufferUtil.EMPTY_BYTE_BUFFER.duplicate();
				b2.get(column_family.getBytes("UTF-8"),0, column_family.length());
				cfMutationMap.put(column_family, v);
				ByteBuffer bb = ByteBufferUtil.EMPTY_BYTE_BUFFER.duplicate();
				bb.get(key.getBytes("UTF-8"));
				batch_mutation.put(bb, cfMutationMap);

				for (String field : values.keySet())
				{
					String val = values.get(field);
					ByteBuffer b = ByteBufferUtil.EMPTY_BYTE_BUFFER.duplicate();
					ByteBuffer bb2 = ByteBufferUtil.EMPTY_BYTE_BUFFER.duplicate();
					bb.get(field.getBytes("UTF-8"));
					bb2.get(val.getBytes("UTF-8"));
	...
	public int delete(String table, String key)
	{
		...
		for (int i = 0; i < OperationRetries; i++)
		{
			try
			{
				ByteBuffer bb = ByteBufferUtil.EMPTY_BYTE_BUFFER.duplicate();
				bb.get(key.getBytes("UTF-8"));
				client.remove(bb, new ColumnPath(column_family), System.currentTimeMillis(),
						ConsistencyLevel.ONE);
